...
sum %>%
  mutate(
    ...
    conf.upper = estimate + 2 * std.error
  )
...
# enter your code here ...
bias = mean(d10$beta1.hat - beta1)
bias

# enter your code here ...
mse = mean((d10$beta1.hat - beta1) ^ 2 )
mse
...
```{r "2_g_2", eval=FALSE, include = FALSE}
ggplot(dat, aes(x=beta1.hat, fill=as.factor(T))) +
...
```{r "3_b_2", eval=FALSE, include = FALSE}
# Type "yes" or "no"
...
# enter your code here ...
bias.hat <- cor(p,eps)*sd(eps)/sd(p)
bias.hat
...
# enter your code here ...
dat <- simulation.study(sim.and.est2,par = list(T=100, price.type=c("random","optimal")), repl=100)
...
library(ggplot2)
ggplot(dat, aes(x=beta1.hat, fill=price.type)) +
  geom_density()
...
```{r "3_h", eval=FALSE, include = FALSE}
dat %>% 
  group_by(price.type) %>%
...
We will come back later in this course to the distinction between predicting a dependent variable and estimating causal effects later.